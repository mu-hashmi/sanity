...
Sanity lets developers and agents produce video-grade run output that a user-facing change actually works, by exercising the running application from a user's seat inside a (Daytona) sandbox running the candidate code.
...
An agent verifier exercises the scenario against the running app.
Sanity gives me a short, reviewable video plus screen recording, logs, and a lightweight RunResult.
Now I (or a reviewer, or an agent) can watch the run output, fix issues, dismiss noisy findings, or share/replay the run.
```
...
- Let an **agent verify its own work** by pointing a verifier at a scenario file and a candidate ref, producing video run output instead of agent-testimony.
- **Attach scenario + RunResult/output reference to a PR** so reviewers can watch the run output without pulling the branch.
...
- Fix a confirmed bug and rerun the scenario to produce video run output of the resolution.
...
- Not a formal merge gate by default. Lead with inspectable run output; gating is a layer that can be added later if a repo wants it.
- Not a hosted service. The durable core is a local CLI + scenario format + sandbox runner + RunContext/RunResult contracts. GitHub Action, PR-attached run output, and hosted review flows are wrappers on top.

**One sentence:**

> Sanity is a sandbox-backed verification tool that lets developers and agents produce short, reviewable video run output of a user-facing change actually working, by running the candidate code in a fresh sandbox and exercising a portable, human-readable scenario against it.
